...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    I = np.identity(3)
    R_cov = I * sensor_var 
    Kk = p_cov_check@h_jac.T@np.linalg.inv(h_jac@p_cov_check@h_jac.T + R_cov)
    ...
    q_hat = Quaternion(axis_angle=delta_phi).quat_mult_right(q_check) 
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    p_est[k] = p_est[k-1] + v_est[k-1]*delta_t + (Cns.dot(imu_f.data[k-1])+g)*(delta_t**2)*0.5

    v_est[k] = v_est[k-1] + (Cns.dot(imu_f.data[k-1])+g)*delta_t
    ...
    F[3:6, 6:9] = Cns.dot(-skew_symmetric(imu)) * delta_t
    ...
    p_cov[k] = F@p_cov[k-1]@F.T + l_jac@Q_k@l_jac.T
